...
def BD_Control_Next(Path_Original, outpath, partitions):
...
def Save_File(Data_Frame, Directory_to_Save, Partitions, Filter_DataBase, Time_File):
    ...
    extension = "csv"
    Name_File = f"Asignacion Multimarca {Time_File}"
    
    output_path = f'{Directory_to_Save}{Type_File}'
    Data_Frame.coalesce(Partitions).write.mode("overwrite").option("header", "true").option("delimiter", ";").csv(output_path)
    ...
    for i, file in enumerate(os.listdir(output_path), start=1):
        if file.endswith(".csv"):
            ...
            new_file_path = os.path.join(output_path, f'{Name_File} {i}.{extension}')
